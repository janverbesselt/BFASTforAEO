...
<<bfastvi, eval=FALSE, fig.width=10, fig.height=10, out.width='.8\\linewidth', fig.cap="BFAST analysis of the cleaned and interpolated NDVI time series", cache=TRUE>>=
...
<<sincos, fig.width=10, fig.height=5, out.width='.8\\linewidth', fig.cap="The harmonic seasonal model", tidy=TRUE>>=
library(bfast)
...
<<eval=FALSE, tidy=TRUE, tidy.opts=list(keep.blank.line=FALSE, width.cutoff=70)>>=
library(raster)
help(projection)
