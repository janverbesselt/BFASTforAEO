...
<<>>=
## you can set the working directory yourself via Rstudio if you go to "session" 
## and then "set working directory"
fluxtoren <- c("fn_nlloobos.txt")  
filenaam <- paste(
...
## if the file exists already in your working directory nothing will happen:
 if(!file.exists(fluxtoren)) {
  download.file(filenaam,fluxtoren)
	modis <- read.csv(fluxtoren, colClasses = "character") 
} else {
	modis <- read.csv(fluxtoren, colClasses = "character") 
}
...
To better understand how bfastmonitor works have a look at the help section of bfastmonitor function and try out the examples provided. 
<<eval=FALSE>>=
help(bfast)
## for more info
