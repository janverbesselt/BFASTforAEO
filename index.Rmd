---
pagetitle: "MODIS NDVI time series analysis using BFAST"
...
# plot the 9th image (time step)
plot(m,9) # plot mask
plot(VI_m,9) # plot cleaned NDVI raster
```

```{block, type="alert alert-success"}
> **Question 2**: Now what would happen if you would only work with "good" quality data (keeping only `QA_r == 0`)? Compare the plots with both good and marginal data to the plots with only good data.
```
...
## Detecting all breaks in the middle of a time series with BFAST Lite

If you need to detect more than one break, then you need to use either BFAST or BFAST Lite.
In this example, we will run BFAST Lite (a lightweight version of BFAST that can handle NA values) on the pixels of the area we were looking at.
...
There is one problem with that: the number of breaks varies from pixel to pixel, but a raster has a fixed number of layers.
...
A run on 4 cores is typically two times faster:
