...
[link](https://vip.arizona.edu/documents/MODIS/MODIS_VI_UsersGuide_June_2015_C6.pdf)
...
```{block, type="alert alert-success"}
> **Question 2**: Now what would happen if you would only work with "good" quality data? Include the new plot of the mask and the cleaned NDVI raster as an answer!
```
...
```{block, type="alert alert-success"}
> **Question 3**: A valid data range of NDVI is between 0 and 1. So we should actually set NDVI values smaller then 0 to NA. Now, do that for this question and run bfastmonitor on this further cleaned NDVI time series.  You can use this R code for that
`tspx[tspx < 0] <- NA` and include the new bfm1 plot in your answer. Describe shortly what happens. How is bfastmonitor analysis influenced by this further cleaning?
```
...
```{block, type="alert alert-success"}
> **Question 4**: Now check if you detect a break in the center pixel (see pixel numbering scheme). What happens if you change the harmonic 'order' from 3 to 1? See `?bfastmonitor`. Can you explain this in two sentences ?
```


```{block, type="alert alert-success"}
> **Question 5**:  What happens if you use a different *formula* in `bfastmonitor`? For example, `response ∼ trend`. Explain what happens and also in which sense is 2019 an abnormal year? Include the bfastmonitor plot in your answer
```


```{block, type="alert alert-success"}
> **Question 6**:  Now try to detect a change in 2019 for another site in the world available via the [https://modis.ornl.gov/sites/](https://modis.ornl.gov/sites/) website. In your answer, mention (a) site, (b) bfastmonitor settings, (c) landcover type of that pixel and the pixel number, (d) and add the plot of the time series. (e) Then explain why you detected a change or why there is no change detected (max 3 sentences). (f) How could the change detection be optimised?
```
