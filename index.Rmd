...
```{block, type="alert alert-success"}
> **Question 3**: What is the key land cover type in the center of the site? (see additional information on the bottom of the [MODIS13Q1 page](https://modis.ornl.gov/cgi-bin/sites/site/?id=nl_gelderland_loobos&product=MOD13Q1))
```
...
neededPackages <- c("strucchange","zoo", "bfast", "raster")
...
```{block, type="alert alert-success"}
> **Question 4**: Now check if you detect a break in the center pixel (see pixel numbering scheme)? Do you detect a break?
```

```{block, type="alert alert-success"}
> **Question 5**: What happens if you change the harmonic 'order' from 3 to 1? See `?bfastmonitor`. Can you explain this in two sentences ?
```


```{block, type="alert alert-success"}
> **Question 6**:  What happens if you use a different *formula* in `bfastmonitor? For example, `response ∼ trend`. Explain what happens and also in which sense in 2018 an abnormal year? 
```


```{block, type="alert alert-success"}
> **Question 7**:  Now try to detect a change in 2018 for another site in the world available via the [https://modis.ornl.gov/sites/](https://modis.ornl.gov/sites/) website. In your answer, mention (1) site, (2) bfastmonitor settings, (3) pixelnumber, (4) landcover type of that pixel and (5) Add the plot of the time series together with explanation of the graph (explain the difference in seasonality between the loobos site and your choosen site)
```
