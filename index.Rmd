...
The following sections give extra information about how to use BFAST Lite and about the concept of seasonality monitoring using harmonic analysis. There are no questions from the following sections, but they may prove to be insightful, especially if you'd like to continue working on time series analysis in the future.
...
In this example, we will run BFAST Lite (a lightweight version of BFAST that can handle NA values) on the same data.
...
library(bfast) # If you get errors about a corrupt database, then you need to restart your R session and rerun the code above
```

Next, let's run the function `bfast0n` (the codename for BFAST Lite) on our data from the previous step:
...
plot(tspx)
abline(v=dates.no.na[breaks$breakpoints[1]], col="red")
lines(fitted(breaks)~dates.no.na, col="blue")
```

Why did the model decide to place a break there? Let's take a look at the statistics:

```{r}
summary(breaks)
```

The above shows that the model tried putting up to 5 breaks in the time series at the particular observation numbers, and selected by minimum BIC that there should be one break. To see that information visually, plot the model:
...
The plot above also shows that the difference between 0 and 1 break is little, therefore the model is not certain which of the two should be picked. In the summary we see that the difference is around 3, which is borderline significant. We can visually see that the break is indeed quite small.
