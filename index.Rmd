...
```{r}
bfm1 <- bfastmonitor(tspx, response ~ trend + harmon, order = 3, start = c(2019,1))
plot(bfm1)
```

```{block, type="alert alert-success"}
> **Question 3**: A valid data range of NDVI is between 0 and 1. So we should actually set NDVI values smaller than 0 to NA. Now, do that for this question and run bfastmonitor on this further cleaned NDVI time series.  You can use this R code snippet for that
`tspx[tspx < 0] <- NA` and include the new bfm1 plot in your answer. Describe shortly what happens. Is this type of cleaning influencing the `bfastmonitor` analysis? Yes or No, explain in one sentence.
```

```{block, type="alert alert-success"}
> **Question 4**: Now check if you detect a break in the center pixel (see pixel numbering scheme). Is there a change detected in the center pixel? Include the `plot(bmf1)` result for the center pixel in your answer.
```
...
```{block, type="alert alert-success"}
> **Question 6**:  Now try to detect a change in 2019 for another site in the world available via the [https://modis.ornl.gov/sites/](https://modis.ornl.gov/sites/) website. In your answer, mention (a) site, (b) bfastmonitor settings, (c) landcover type of that pixel and the pixel number, (d) and add the plot of the time series. (e) Then explain why you detected a change or why there is no change detected in an example time series for a selected pixel. (f) How could the change detection be optimised?
```

Example R code to select pixel select with a break detected by using the `click` function and click on pixels with a break (i.e. where a time of break is estimated).
```{r, eval = FALSE}
plot(bfmR,1)
click(VI_m, id=FALSE, xy=FALSE, cell=TRUE, n= 1)
```

Here we selected one pixel, and do the `bfastmonitor` analysis for that pixels:
```{r, eval = TRUE}
